...
const TickerItem = ({ symbol, price, change, isPositive = false }: { symbol: string, price: string, change: string, isPositive?: boolean }) => (
    <span className="inline-block px-4">
        <span className="text-xs font-mono text-white/50">
            <span className="text-white/50 bg-white/10 px-2 py-1 rounded">{symbol}</span> {price} <span className={isPositive ? "text-green-500" : "text-red-500"}>{isPositive ? '▲' : '▼'} {change}</span>
        </span>
    </span>
);
